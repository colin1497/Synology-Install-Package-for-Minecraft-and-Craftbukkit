...
if [ "${SYNOPKG_PKGNAME}" == "Minecraft" ]; then
  DOWNLOAD_PATH="http://s3.amazonaws.com/Minecraft.Download/versions/1.10.2"
  DOWNLOAD_FILE="minecraft_server.1.10.2.jar"
  UPGRADE_FILES="server.properties *.txt world *.json"
fi
if [ "${SYNOPKG_PKGNAME}" == "Craftbukkit" ]; then
  DOWNLOAD_PATH="https://ci.mcadmin.net/job/CraftBukkit/279/artifact"
  DOWNLOAD_FILE="craftbukkit-1.10.2.jar"
...
if [ "${SYNOPKG_PKGNAME}" == "Spigot" ]; then
  DOWNLOAD_PATH="https://ci.mcadmin.net/job/Spigot/91/artifact"
  DOWNLOAD_FILE="spigot-1.10.2.jar"
